...
# 3. 配置环境变量（添加到 ~/.bashrc）
export VCS_HOME=/opt/synopsys/vcs/latest
...
```bash
# Verdi通常与VCS一起安装
export VERDI_HOME=/opt/synopsys/verdi/latest
...
### QuestaSim安装（Mentor/Siemens）

```bash
# 1. 运行安装程序
chmod +x questa_sim_*.run
sudo ./questa_sim_*.run

# 2. 配置环境变量
export QUESTA_HOME=/opt/questasim/latest
export PATH=$QUESTA_HOME/bin:$PATH
export LM_LICENSE_FILE=1717@your_license_server:$LM_LICENSE_FILE
...
2. **Synopsys VCS文档**: `$VCS_HOME/doc/`
3. **QuestaSim文档**: `$QUESTA_HOME/docs/`
4. **Verdi用户手册**: `$VERDI_HOME/doc/`
